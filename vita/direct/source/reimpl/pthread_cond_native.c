...
/* Vita port modification: release the process-wide condition-list lock on
 * BOTH error exits. A busy condition must not block all later condition init
 * and destruction. Kept as a link wrapper so SDK users share the same fix.
 * Upstream: vitasdk/pthread-embedded, pthread_cond_destroy.c (see docs). */
#include <stdlib.h>

#include "reimpl/pthread_cond_native.h"
