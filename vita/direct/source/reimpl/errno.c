...
static __thread int g_errno_bionic_tls = 0;
...
static int translate_newlib_errno_to_bionic(int e) {
	...
	// Preserve unknown values instead of silently converting them to success (0).
	l_warn("Unexpected newlib errno %i, passing through", e);
	return e;
}

int * __errno_soloader(void) {
	g_errno_bionic_tls = translate_newlib_errno_to_bionic(errno);
	return &g_errno_bionic_tls;
}
...
char * strerror_soloader(int error_number) {
	...
	l_warn("Unexpected bionic errno %i, returning generic message", error_number);
	snprintf(unknown_buf, sizeof(unknown_buf), "Unknown error %d", error_number);
	return unknown_buf;
}
