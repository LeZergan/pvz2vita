#ifndef PVZ2_CACHE_CRC_H
#define PVZ2_CACHE_CRC_H
#include <stdint.h>
#include <stddef.h>
/* IEEE CRC-32, slicing by four. Immutable 4 KiB tables; no initialization,
 * allocation, alignment assumptions or change to the on-disk CRC format. */
static const uint32_t pvz2_crc_table[4][256] = {
    {
...
static __attribute__((noinline)) uint32_t pvz2_cache_crc32(uint32_t crc, const unsigned char *p, size_t n) {
    crc = ~crc;
    while (n >= 4) {
        uint32_t word = (uint32_t)p[0] | ((uint32_t)p[1] << 8) |
                        ((uint32_t)p[2] << 16) | ((uint32_t)p[3] << 24);
        crc ^= word;
        crc = pvz2_crc_table[3][crc & 255] ^ pvz2_crc_table[2][(crc >> 8) & 255] ^
              pvz2_crc_table[1][(crc >> 16) & 255] ^ pvz2_crc_table[0][crc >> 24];
        p += 4; n -= 4;
    }
    while (n--) crc = (crc >> 8) ^ pvz2_crc_table[0][(crc ^ *p++) & 255];
    return ~crc;
}
#endif
