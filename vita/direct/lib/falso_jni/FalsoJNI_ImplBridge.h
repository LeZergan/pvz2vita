...
/* Per-method stub registry for boolean-returning unknown methods, so a polled
 * boolean's NAME is logged instead of the shared FJNI_STUB_METHOD_ID. */
#define FJNI_STUB_BOOL_BASE 0x7E000000
jmethodID   fjni_register_boolstub(const char* name);
const char* fjni_boolstub_name(jmethodID id);
/* Generalized: name-track ANY unknown method (type: 'Z'/'I'/'S'/'L'/'V'). */
...
#define GetPrimitiveArrayRegion(fun_name, fieldType, jType, array, start, length, buffer) ({ \
    ...
    if (!jda) { \
     ...
     \
    if (start < 0 || length < 0 || start > jda->len || (start + length) > jda->len) { \
