...
int pthread_create_soloader(pthread_t *t, const pthread_attr_t_bionic *a,
    ...
    return pthread_create(t, NULL, f, v);
}
...
int main(int argc, char **argv) {
    ...
    pvz2_pixels_init(3);
    assert(worker_count == (unsigned)create_limit);
    ...
    assert(jobs>30);
    if(create_limit) assert(parallel_jobs && worker_pixels && caller_pixels);
...
for workers in range(4):
    run=subprocess.run([str(exe),str(workers)],check=True,capture_output=True,text=True,timeout=8)
